...
const fetchData = async (exchange, marketFilter, sortType) => {
  ...
  try {
    ...
    if (jsonData) {
      const formattedData = {
        exchange: exchange,
        marketFilter: mapMarketFilter(marketFilter),
        sortType: sortType,
        result: {
          symbol_codes: [],
          company_names: [],
      ...
      if (jsonData.length > 0) {
        ...
        formattedData.result.movements = jsonData.map((row) =>
          row.movement !== null ? row.movement.toString() : "0"
        );
        ...
        formattedData.result.total_volumes = jsonData.map((row) =>
          row.total_volume !== null ? row.total_volume.toString() : "0"
        );
      }
    ...
    console.error(
      `Error fetching data for ${exchange}, ${marketFilter}, ${sortType}:`,
      error.message
    );
    return null;
  }
    ...
    for (const marketFilter of Object.values(config.MARKET_FILTER1)) {
      for (const sortType of Object.values(config.SORT_TYPE)) {
        ...
        if (result) {
          results.push(result);
          console.log(
            `Fetched data for ${exchange}, ${result.marketFilter}, ${sortType}`
          );
