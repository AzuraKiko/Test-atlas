...
async function callCallbackAPI(requestUid) {
...
async function runTestCases() {
    for (const testCase of testCases) {
        ...
        try {
            ...
            // Nếu API chính thành công, lưu lại request_uid
            const requestUid = response.data.request_uid;

            // Gọi API callback ngay lập tức sau khi gửi yêu cầu
            callbackResponse = await callCallbackAPI(requestUid);
            ...
            if (error.response) {
                ...
                // Gọi API callback ngay cả khi có lỗi từ API chính
